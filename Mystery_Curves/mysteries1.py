...
def f(t, alpha, beta, gamma):
    return (alpha * np.exp(7j*t)) + (beta * np.exp(31j*t)) + (gamma * np.exp(23j*t)) + (delta * np.exp(101j*t) )
...
for frame in range(frames+1):
    ...
    #for index in range(len(Z) - 1 ):
        ...
        #plt.plot([np.real(Z[index]),np.real(Z[index + 1])],[np.imag(Z[index]),np.imag(Z[index+1])],'-',alpha = 1 , lw=0.2, color = plt.cm.hsv( percent ) )
