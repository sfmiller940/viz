...
Xs, Ys = dynsine(15,40,20000000,0,2.8,4.0)    
    
def histogrammar(dpi, colormaps):    
    Hist, Xedges, Yedges = np.histogram2d( Xs, Ys, bins=(6000, 5000))
    Hist = np.log1p(  Hist )
    ...
    for i, colormap in enumerate(colormaps):
        ...
        plt.imshow(Hist, interpolation='bicubic', cmap=plt.cm.get_cmap(colormap), origin='lower', extent=[0,1.2,0,1])
        f80.savefig('logistprint_' + colormap + '.png',dpi=dpi, facecolor='w', edgecolor='w', bbox_inches='tight', pad_inches=0)
        plt.close( f80 )


#Example
histogrammar(2500, ['Greys', 'gray_r','bone_r','gist_ncar_r','Accent','afmhot','binary','cubehelix','gist_rainbow_r','hsv_r','ocean_r','seismic','Set1','Set3','terrain_r','YlGnBu'])
